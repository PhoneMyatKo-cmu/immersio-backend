...
def save_user_vocab_exposure(session_data: LearningSession, db: Session):
    ...
    for vocab_profile in video_vocab_profiles:
        ...
        caption_indices = vocab_profile.caption_indices.split(",") if vocab_profile.caption_indices else []
        for index_str in caption_indices:
            ...
            if 0 <= index < len(captions):
                ...
                if not caption:
                    raise ValueError(f"Caption with index {index} not found for video {video_id}.")

                for interval in intervals:
                    start_time = interval.start_time
                    end_time = interval.end_time

                    if isIntervalOverlapping(caption.start_time, caption.end_time, start_time, end_time):
                        vocab_exposure = next((
                            (exposure for exposure in existing_exposures if exposure.vocab_id == vocab_profile.vocab_id)
                        ), None)
                        if vocab_exposure:
