...
def get_home_feed(db: Session = Depends(get_db),
                ...
                page: int = 1,
                page_size: int = 6):
    videos, total_videos = get_videos(db, search=search.lower() if search else None, page=page, page_size=page_size)
...
def get_videos_by_difficulty(db: Session = Depends(get_db),
                            difficulty_level: DifficultyLevel = DifficultyLevel.beginner,
                            search: str = None,
                            page: int = 1,
                            page_size: int = 6):
    videos, total_videos = get_videos_by_difficulty_level(search, difficulty_level, db, page, page_size)
    total_pages = (total_videos + page_size - 1) // page_size
    return videos, total_pages
